        ...
        <nav className="hidden md:flex space-x-4 lg:space-x-10 text-[#474749] font-semibold text-sm md:text-base">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="relative after:absolute after:bottom-0 after:left-0 after:w-0 after:h-[2px] after:bg-[#474749] after:transition-all after:duration-300 hover:after:w-full"
            >
              {link.label}
            </a>
          ))}
